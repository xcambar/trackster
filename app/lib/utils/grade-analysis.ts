...
export function analyzeGradePerformance(streams: ActivityStreams[]): GradeAnalysis[] {
  const gradeData: { [gradeRange: string]: { totalTime: number; totalDistance: number } } = {};
  ...
  streams.forEach(stream => {
    ...
    for (let i = 0; i < grades.length; i++) {
      ...
      if (grade !== null && grade !== undefined && velocity !== null && velocity !== undefined && velocity > 0) {
        const gradeRange = categorizeGrade(grade);
        
        // Calculate time interval (assume 1 second intervals if not specified)
        const timeInterval = i < times.length - 1 && times[i + 1] !== null && times[i] !== null 
          ? (times[i + 1]! - times[i]!) : 1;
        const distanceInterval = i < distances.length - 1 && distances[i + 1] !== null && distances[i] !== null
          ? (distances[i + 1]! - distances[i]!) : velocity * timeInterval;
  ...
  return GRADE_RANGES.map(range => {
    ...
    return {
      ...
      averagePaceMinPerKm: data && data.totalDistance > 0 
