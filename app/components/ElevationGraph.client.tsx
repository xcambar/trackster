...
export const ElevationGraph: React.FC<ElevationGraphPropsUnion> = React.memo(
  (props) => {
    ...
    const elevationData: ElevationDataPoint[] = React.useMemo(() => {
      ...
      const calculateGradeAtDistance = (
        ...
        for (let i = 1; i < points.length; i++) {
          ...
          const p2 = points[i];
          if (p1 && p2) {
            const segmentDistance = distanceFunc(
              p1.lat,
              p1.lng,
              p2.lat,
              p2.lng
      ...
      const calculateEffortRate = (
        ...
        // Base effort from grade (using realistic physiological demands)
        let baseEffort: number;
        if (grade >= 0) {
          // Uphill: exponential increase in effort
          baseEffort = 1 + Math.pow(grade / 100, 1.5) * 8;
        } else {
          // Downhill: less effort but not zero (quad impact, braking)
          const downhillGrade = Math.abs(grade);
          baseEffort = 1 + Math.pow(downhillGrade / 100, 1.2) * 3; // Less impact than uphill
        }
    ...
    const gradeRanges = React.useMemo(() => {
      ...
      const totalRed = ranges.red.reduce(
        (sum, r) => sum + (r.end - r.start),
        0
      );
